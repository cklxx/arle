...
- `arle` 是用户前门：本地 agent、`serve`、训练 / 评测与数据工具。
- `infer` 是给运维和集成方直接使用的专用 serving 二进制。
...
./target/release/arle --model-path /path/to/Qwen3-4B run --no-tools --prompt "不使用工具总结一下这个仓库"
...
而 `arle run --prompt ...` / `--stdin` 则是更适合脚本的单次执行路径。需要保证
prompt 不执行内置 shell/python 工具时，使用 `--no-tools`。
...
发布包命名为 `arle-<version>-<platform>.tar.gz`，包含 `arle` 前门与匹配的
serving 二进制。
...
**前置要求**：[`rust-toolchain.toml`](rust-toolchain.toml)
里固定的 Rust 工具链（当前 `1.95.0`）。默认 CUDA 构建额外要求 CUDA 12.x，
以及 Python 3.10+ 带 `flashinfer-python`（仅构建期使用）。
`crates/kv-native-sys` 需要的 Zig `0.16.0` 会由
[`scripts/setup_zig_toolchain.sh`](scripts/setup_zig_toolchain.sh) 和
`./setup.sh` 自动引导。Metal 和纯 CPU 的 CLI 构建请直接使用上面的特性化命令，
不要走默认 CUDA 特性集。
...
- `arle serve` — 统一前门，负责从 release artifact 或 PATH 启动匹配的
  serving 二进制（`infer`、`metal_serve` 或 `cpu_serve`）。
...
- [examples/](examples/) — curl、Docker、Metal 与 tiny train fixture 的可复制示例
...
装了 `nsjail` 时继续使用。
`arle --doctor` 会报告当前工具沙箱后端；如果显示为 `bare`，处理不可信
prompt 时请使用 `--no-tools`。
