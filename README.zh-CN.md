...
所以 ARLE 不是“一个推理引擎外加几段脚本”。这条推理主干本身就是更完整
agent RL 闭环的地基：同一套 Rust 模型 / 运行时权威、同一 workspace 内
的训练二进制，以及一个顶层 CLI，既能当本地 agent，也能作为 train /
eval / data 的统一前门，不需要再绕一层额外的 Python 控制平面。
...
下面公开展示的数字目前仍然以 serving 侧为主，因为正在做的是 CUDA 对
SGLang 的收口。项目本身的外部表面更宽：同一套运行时权威也支撑本地
`arle` agent 前门，以及仓内的 train/eval 栈。
