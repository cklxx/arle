...
- **2026-04-22** — CUDA `Qwen3.5` 现在走真正的 packed multi-request paged-prefill 路径。全注意力层直接写 paged pool，混合线性注意力层接上 packed recurrent-state 发射，paged-prefill logits 也回到统一采样面。执行记录见 [`docs/plans/2026-04-22-sglang-gap-closure-execution.md`](docs/plans/2026-04-22-sglang-gap-closure-execution.md)。
- **2026-04-22** — CUDA 调度重叠继续收紧：decode launch/readback 拆到跨迭代阶段，fetch wait 改成事件驱动，streaming emit 搬到独立 emit worker，并把 gate 结果再喂回调度循环。运行时权责图见 [`docs/projects/tiered-kv-runtime-flow.md`](docs/projects/tiered-kv-runtime-flow.md)。
...
工具链。当前运行时主线仍然是 CUDA-first 的 serving path，但项目身份已经
不只是一个独立的推理二进制。
...
ARLE（agent reinforcement learning engine）把这件事当成核心问题：
...
而这条推理主干，也是 ARLE 向更完整 agent RL 闭环扩展的基础：同一套 Rust
模型 / 运行时权威、同一 workspace 内的训练二进制，以及一个顶层 CLI，
既能当本地 agent，也能作为 train / eval / data 的统一前门，不需要再绕
一层额外的 Python 控制平面。
...
`infer` 是服务二进制；`arle` 是 ARLE 这套 workspace 面向 agent、train /
eval 与 dataset 工具链的统一前门。
