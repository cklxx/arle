...
> **TL;DR:** infer led the measured Qwen3-4B workloads on RTX 5070 Ti after the FlashAttention-2 + PrefillBuffers work. This doc is archived as a dense-attention optimization reference rather than an active project.
>
> **Status:** Archived. Kept as a reference for dense-attention optimization work; no longer an active milestone.

## Goal

infer single-request latency >= vLLM on the same GPU, model, and workload. No regressions allowed — decode must stay at parity while prefill catches up.

## Current (2026-03-13, post FlashAttention-2 + cuBLAS workspace)

GPU: RTX 5070 Ti, Model: Qwen3-4B, vLLM 0.17.1, single concurrency.
infer: in-process bench_serving (no HTTP overhead). vLLM: `vllm bench serve` HTTP.

### in=1, out=1 — Minimal overhead

| Metric | infer | vLLM | delta |
|--------|-----------|------|-------|
...
Verdict: **infer wins.** Prefill 14% faster (was -5% in pre-FA2 baseline).
...
Verdict: **prefill at parity, edge to infer.** Pre-optimization was 270ms (+103% vs vLLM). After FlashAttention-2: 213ms (-7% vs vLLM).
...
**Lesson:** True GQA-aware FA needs explicit SMEM (CUDA kernel, not Triton). Triton doesn't expose SMEM for manual tile staging. With infer already 7% faster than vLLM, the complexity cost is unjustified. Reverted.
