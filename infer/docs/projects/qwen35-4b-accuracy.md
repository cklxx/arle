...
- `tests/gen_test_data_35.rs` regenerates that JSON from infer itself.
...
- `docs/projects/qwen35-4b-optimization.md` records that the refreshed chunk-wise baseline changed `6/13` prompts. Once such a baseline is accepted, the current e2e no longer tells us where infer differs from HF.
...
- New accuracy tooling now exists for layer `0` prefill:
  - `src/bin/qwen35_dump_layer0.rs` dumps infer layer-0 checkpoints to JSON
...
Most importantly, this is not just an HF mismatch. infer decode is also inconsistent with infer prefill:
...
After replacing the HD256 decode attention kernel with the validated prefill path, first-decode HF mismatch improved but did not disappear. The next step was to compare incremental decode against fresh full-prefill inside infer itself.
...
- because [test_data/Qwen3.5-4B.json](/data/code/workspace-rustllm/infer/test_data/Qwen3.5-4B.json) is the old self-generated baseline
- a fresh candidate baseline was generated to:
  - [target/accuracy/Qwen3.5-4B.current.json](/data/code/workspace-rustllm/infer/target/accuracy/Qwen3.5-4B.current.json)
...
- HF exact-token-id prefill on that prefix has max-logit tokens:
  - `[32, 1206]`
- infer exact-token-id prefill on that prefix has max-logit tokens:
  - `[727, 1206]`
...
- infer's production decode path currently does **not** run the old per-layer decode kernels
...
Crucially, this later-layer gap is not unique to infer. On the same exact prefix, HF's own incremental decode also separates from HF full-prefill by a similar amount at layer `31`:
...
infer shows the same qualitative pattern on the same step:
