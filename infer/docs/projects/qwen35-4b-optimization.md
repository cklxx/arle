...
infer single-request latency >= vLLM on Qwen3.5-4B, same GPU/workload. The original prefill-heavy gap is now mostly closed: chunk-wise GDR prefill gets `(2048,1)` TTFT to parity level on this GPU, so the remaining work is normal tuning and cleanup rather than a structural latency crisis.
...
**E2E numbers:**
- Prefill-heavy (2048,1): infer 16,846ms vs vLLM 222ms (**76× slower**)
- Decode-heavy (1,128): infer 12.55ms vs vLLM 11.64ms (+8%)
