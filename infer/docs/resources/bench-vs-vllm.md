# infer vs vLLM Comparative Benchmarking
...
VLLM_PYTHON=.venv/bin/python      # infer/.venv with vllm installed
...
Prerequisites:
- `cargo build --release` — infer up-to-date
...
- **infer empty prompts:** Random dataset may produce empty prompts which infer rejects. Check failed request count.
- **Zombie processes:** Always `pkill` after benchmarking. Leftover servers block the port and hold GPU memory.
- **CUDA Graph:** infer enables CUDA Graph by default. For apples-to-apples decode comparison, note this in the report. vLLM also uses CUDA Graph by default.
- **Concurrency=1:** Default is single-request sequential. infer has no batching yet, so concurrency>1 would just queue on the server side.
