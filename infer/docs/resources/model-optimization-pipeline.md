...
infer: `bench_serving request --prompt-len <in> --output-len <out>` (see [profiling-guide](../resources/profiling-guide.md)). vLLM: `vllm bench serve` (see [bench-vs-vllm](../resources/bench-vs-vllm.md)).
...
Current infer vs vLLM end-to-end numbers. **Update the infer column after each optimization** to always reflect the latest state. The vLLM column only needs to be measured once at baseline — re-measure when upgrading vLLM.
...
| Profile | Metric | infer | vLLM | delta |
|---------|--------|-----------|------|-------|
...
Expand the model's computation graph first, then annotate infer's support status.
