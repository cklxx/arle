    ...
    mlx_array* result_out)
{
    ...
    array q_heads = slice(q_full, {0,0,0,0}, {1,1,n_heads,head_dim});
    array gate_heads = slice(q_full, {0,0,0,head_dim}, {1,1,n_heads,head_dim*2});
    ...
    k_cache = slice_update(k_cache, k, kv_start, kv_end);
    v_cache = slice_update(v_cache, v, kv_start, kv_end);

    std::vector<int> kv_origin = {0, 0, 0, 0};
    array k_full_cache = slice(k_cache, kv_origin, kv_end);
    array v_full_cache = slice(v_cache, kv_origin, kv_end);
    ...
    mlx_array* result_out)
{
    ...
    array conv_out = sum(multiply(full_window, conv_weight), /*axis=*/{1});
    ...
    array q_per_head = reshape(q_raw, {num_key_heads, key_dim});
    array k_per_head = reshape(k_raw, {num_key_heads, key_dim});
    ...
    array k_3d = reshape(k_expanded, {num_value_heads, key_dim, 1});
    array kv_mem = sum(multiply(s_decayed, k_3d), {1}); // [H, V]
    ...
    array q_3d = reshape(q_expanded, {num_value_heads, key_dim, 1});
