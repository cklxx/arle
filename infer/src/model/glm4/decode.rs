...
impl GLM4Model {
    ...
    fn decode_layer_inner(
        ...
        // QKV projections: use separate projections for decode (GEMV)
        ops::gemv(
            &self.ctx,
        ...
        // Add QKV bias: use pre-split bias vectors from weight loading.
        ops::vec_add_inplace(&self.ctx, &mut bufs.q, &layer.attention.q_bias)?;
        ops::vec_add_inplace(&self.ctx, &mut bufs.k, &layer.attention.k_bias)?;
        ops::vec_add_inplace(&self.ctx, &mut bufs.v, &layer.attention.v_bias)?;

        // Attention: GLM-4 has no Q/K norm — use dummy identity norm weights.
