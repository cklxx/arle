...
| `backend/cuda.rs` | **Thin re-export shim** for `cuda_kernels::{ffi, flashinfer, paged_kv, prelude, tensor, collective, ...}`. ~15 lines. Historical `crate::backend::cuda::...` paths still resolve through this. Only `bootstrap.rs` is a real module, and it only exists with `#[cfg(feature = "cuda")]`. The bootstrap path also owns multi-rank NCCL init / device selection for the P0' single-node multi-GPU scaffold. |
