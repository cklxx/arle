...
//!   (private module; no intra-doc link)
...
impl Coordinator {
    ...
    // Internal handler for `CoordinatorHandle::submit_prefetch_plan`.
    // See the rustdoc on that public method for the reserved-API
    // rationale (status, why-not-now, when-it-becomes-useful). Keeping
    // the canonical explanation on the public entry point so rustdoc
    // links resolve cleanly under `RUSTDOCFLAGS='-D warnings'`.
    fn handle_plan(
        &self,
        ticket: PlanTicket,
