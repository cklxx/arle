...
| --- | --- | --- |
| **Projects** | | |
| [projects/tiered-kv-cache.md](projects/tiered-kv-cache.md) | **Active — M2b local shipped** | Hierarchical KV cache (T0 GPU → T1 host pinned → T2 NVMe → T3 NIXL). M1+M2a+M2b local landed; remote CUDA acceptance pending. M3 (coordinator + page lifecycle + policy convergence + recompute fallback) next |
