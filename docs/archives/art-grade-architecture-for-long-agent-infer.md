...
│   ├── chat/                 # 协议类型
│   ├── tools/                # builtin tools + shared tool policy hooks
│   ├── agent/                # agent loop（不拥有 UI / runtime 私有细节）
...
```text
infer-core      infer-policy      infer-observability      chat
    ↑                ↑                    ↑                   ↑
    └──── tools / agent / infer-engine ───────────┘
                           ↑
                        cli
                           ↑
                      agent-infer
...
约束：
1. `infer-engine` 不得反向依赖 `agent` / `cli`。
2. `agent` 不得拥有 UI 输出，也不直接拥有 runtime/backend 私有细节。
3. `tools` / `chat` 提供共享协议与默认语义，但不反向感知 CLI。
...
1. 保持已拆出的控制面 crate 可编译、可测试、可回退：`agent`、`cli`、`chat`、`tools`、`infer-core`、`infer-engine`、`infer-observability`、`infer-policy`。
...
**Phase 1 PR 切分纪律**：
- 单个 PR 只允许一个主主题：数值正确性、scheduler 语义、backend 能力、workspace 结构，这几类不得混在同一 review 面里。
- 如果一次改动同时涉及“行为变化”和“结构变化”，必须先交行为 PR，再交结构 PR；结构 PR 只能做边界搬移和适配，不再顺手修 kernel / scheduler 语义。
- 计划文档先于实现定稿：crate 准入条件、影响层级、依赖方向要先写清，再开始搬代码；实现后的文档只允许更新状态，不允许反过来追认边界。
- 新 crate 的 promotion PR 要么在同一组 stacked PR 里证明至少两个直接消费者，要么继续留在 `infer/` 内部模块，不允许靠“未来会复用”预支拆分。
...
6. 任何新 crate 都要在 PR 描述里明确写出两个直接消费者；写不出来就说明还不该拆。
7. 任何 branch 在打开 review 前都要整理成单主题提交序列；如果 reviewer 需要同时理解 kernel、scheduler、workspace 三种语义，说明切分已经失败。
