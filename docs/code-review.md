...
模块边界清晰：
- `pegainfer` 只暴露 `ModelForward`、`ServerEngine`、`SchedulerHandle` 三个核心 trait/类型
- `src/` 依赖 `pegainfer`，不反向依赖
...
- `pegainfer/src/scheduler.rs`：`step_new()` 命名不够语义化，应为 `step_prefill_new()` 或 `step_new_request()`，与同文件的 `step_prefill_chunk()`、`step_decode_batch()` 对齐。

- `agent_infer/mlx_backend.py`：`_build_sampling()` 返回一个 tuple，但命名暗示只构建 sampler。应改为 `_build_sampler_and_processors()` 或拆成两个函数。

- `pegainfer/src/model/qwen35/decode_buffers.rs`：文件名用 `decode_buffers`，但 qwen3 目录里对应文件叫 `decode.rs`，命名不一致。
...
**`pegainfer/src/scheduler.rs:596`**
```rust
...
**`pegainfer/src/scheduler.rs:755`**（相同模式）
```rust
*active[i].generated_tokens.last().unwrap()  // ⚠️
```

**`pegainfer/src/http_server.rs:100, 109-110, 218, 231, 240`**
```rust
...
**`pegainfer/src/server_engine.rs:270-272`**
```rust
...
**`pegainfer/src/main.rs:59, 82, 97, 129`**
```rust
...
```rust
// pegainfer/src/model/qwen3/forward.rs:26
...
**Rust 测试：**
- `pegainfer/src/ops/tests.rs`：~1171 行，覆盖张量算子（CPU 路径） ✅
- `pegainfer/src/model/qwen3/`：配置解析、权重加载单测 ✅
- `pegainfer/src/sampler.rs`：采样参数单测 ✅
- `pegainfer/src/prefix_cache.rs`：前缀匹配逻辑单测 ✅
- `pegainfer/src/block_manager.rs`：内存块分配单测 ✅
...
**`pegainfer/src/http_server.rs:223, 238`**
```rust
...
**`pegainfer/src/prefix_cache.rs`**：前缀匹配使用 radix tree，O(k) 查找（k 为前缀长度），合理 ✅

**`pegainfer/src/scheduler.rs`**：请求队列遍历为 O(n)，n 为并发请求数。在高并发（>1000）场景下可能成为瓶颈，但当前设计目标是几十到几百请求，可接受 ✅

**`pegainfer/src/block_manager.rs`**：块分配使用空闲链表，O(1) 分配，O(1) 回收 ✅
...
**`pegainfer/src/http_server.rs`** 有基本验证：
...
**位置：** `pegainfer/src/scheduler.rs:596, 755`
...
**位置：** `pegainfer/src/http_server.rs`
...
**位置：** `pegainfer/src/model/qwen3/forward.rs:26`, `qwen35/forward.rs:26`, `cuda_graph.rs:18`
...
**位置：** `pegainfer/src/http_server.rs:41-285`
