...
| M5.3 | ⚠️ 部分完成：matmul 已上 Metal（3-shape 对拍 ≤ 1e-3，commit `a46fc00`）；add/mul_scalar/sum/log_softmax/gather/AdamW 仍走 CPU（`Backend::matmul_forward` 是 trait 里唯一方法） | 每个剩余 op 数值对拍 CPU f64 参考 ≤ 1e-3。**注 2026-04-19**：`docs/experience/wins/2026-04-18-bench-train-multi-turn.md` 实测 TinyLM 尺度下 Metal matmul 比 CPU 慢 1.9×（per-call FFI + 图构造开销主导），只有 d_model≥256 才领先 1.4×。扩展剩余 op 到 Metal 之前需要先决定：是否直接上 device-resident tensor 消除 per-call upload/download，否则 M5.3 扩展 + M5.4 demo 都会被相同 FFI 瓶颈限制。**MLX 上游佐证（2026-04-19 web 调研）**：[MLX lazy-eval 文档](https://ml-explore.github.io/mlx/build/html/usage/lazy_evaluation.html) 明确 "there is some fixed overhead with each graph evaluation" 并将 "tens-to-thousands of ops per evaluation" 列为推荐图尺度。当前 `Backend::matmul_forward` 每 call 构造 1-op 图 + eval + readback，恰好命中上游认定的 degenerate 模式；device-resident tensor + 多 op 合并 eval 是 MLX 原生模式而非"优化方向"。结论：M5.3 扩展路线应为 (i) `GpuTensor` 内部持 `Option<mx::Array>`（lazy），(ii) 上层 op 组图不 eval，(iii) `backward()` 或 `optimizer_step()` 触发一次 eval 覆盖一轮前后向。优先级重排讨论再启动。 |
