...
| M5.3 | 🟢 **M5.3a LANDED 2026-04-20** — device-resident tensor + lazy eval boundary shipped. Commits `17b3599`（M5.3a.1: Tensor.device_handle + Dirty enum + ensure_host/ensure_device）、`7d024d8`（M5.3a.2: lazy matmul + add）、加 matmul-backward-on-GPU follow-up（见 `docs/experience/wins/2026-04-20-matmul-backward-gpu.md`）。验收：`cargo test -p autograd --release --features metal` 48 路全绿，`metal_single_forward_backward_step_has_bounded_eval_count` 断言 forward matmul=0 eval / sum=1 eval / backward ≤2 eval（对照 pre-M5.3a 每 op 一次 eval 的退化路径）。bench_step_matmul 批量 32 时 d_model=128 Metal = 1.74× CPU（gate ≥1.1×），d=256 5.78×，d=512 12.34×；批量 4 仍 CPU 主场（d=128 0.42×）。详细规格：[`docs/plans/m5.3-device-resident-tensor.md`](m5.3-device-resident-tensor.md)；验收快照：[`docs/experience/wins/2026-04-20-m5.3a-device-resident-tensor-acceptance.md`](../experience/wins/2026-04-20-m5.3a-device-resident-tensor-acceptance.md)。**M5.3b 进行中**：✅ M5.3b.1 `sum_all` lazy（commit `b28aa36` 前后）。✅ M5.3b.2 `softmax`/`log_softmax` lazy forward（commit `93447e0` — forward 链 `matmul → log_softmax → sum` 0 eval，`metal_log_softmax_forward_backward_records_current_eval_cost` 断言 after_log_softmax==0 / after_backward≤3）。✅ M5.3b.3 `silu` lazy forward（commit `875c935` — `mlx_sigmoid * x` 组合，MLP 每层 2 次 silu 从 eager→lazy，Qwen3.5 28 层潜在减少 56 evals/token；新增 `metal_silu_forward_stays_lazy` 断言）。✅ M5.3b.4 `exp` lazy forward（`mlx_exp` 单节点接入 MLX graph，Dirty-dispatch 同 silu 模板；`exp_backward` 依赖 `tape.backward` 的 pre-walk batch-flush 读 saved output，forward 保持 lazy 仍安全；新增 `metal_exp_forward_stays_lazy` 断言 forward 链 0 eval）。✅ M5.3b.5 `rope` lazy forward（`mlx_slice` → `mlx_multiply` → `mlx_subtract`/`mlx_add` → `mlx_concatenate_axis` 全链 lazy，借 `x_handle` 不重新 upload；cos/sin 仍 host 侧 upload 每次；dispatch 放宽到 `device_handle.is_some() && dirty != Host`，兼顾 Dirty::Device 和 Dirty::Both（ensure_device 后重入场景）；Qwen3.5 每层 q/k 两次 rope × 28 层 = 56 次 eval/token 潜在消除；新增 `metal_rope_forward_stays_lazy` 断言 forward 0 eval + CPU 参考对拍 ≤ 1e-5）。✅ M5.3b.6 `rmsnorm` lazy forward（`mlx_fast_rms_norm` 单节点，weight 每次 upload 成 `[hidden]` 临时 array；`SavedContext::RMSNormCtx` 加 `eps: f32` 字段，lazy 路径存空 `inv_rms` vec 作 sentinel，backward 直接用已 destructure 的 eps 从 host-flushed x 重算 inv_rms；Qwen3.5 每层 3 次 rmsnorm（input/post-attn/post-mlp）× 28 层 = 84 evals/token 潜在消除；新增 `metal_rmsnorm_forward_stays_lazy` 断言 forward 链 `matmul → rmsnorm → sum` 0 eval + CPU 参考对拍 ≤ 1e-4）。✅ M5.3b.7 `embedding` lazy forward（`mlx_take_axis(table_handle, ids_i32, axis=0) → mlx_reshape [seq,hidden]→[1,seq,hidden]` 组合，ids 每 step upload 成 `[seq]` int32 临时 array；dispatch 同 rope 模板覆盖 Dirty::Both，故 AdamW 上传新表后下一次 forward 仍走 lazy；embedding 是 forward pass 第一个 op，上游 lazy 打开后全链从 token id 起就驻留在设备；新增 `metal_embedding_forward_stays_lazy` 断言 `embedding → rmsnorm → sum` 链 0 eval + CPU 参考对拍 ≤ 1e-5；backward 仍走 host scatter-add，依赖 tape 批量 flush 提供 host upstream）。✅ M5.3b.8 `gelu` lazy forward (erf 形式，`mlx_multiply(INV_SQRT_2, x) → mlx_erf → mlx_add(1, ·) → mlx_multiply(0.5x, ·)` 组合；关键：用 erf 而**非** `gelu_forward` 的 tanh 近似，因为 `gelu_backward` 用 saved `x` 算 erf 导数，forward 若切 tanh 会和 backward 偏离 ~1e-3；新 `Backend::gelu` device 方法正好就是 erf 契约，tanh `gelu_forward` 留给别处；新增 `metal_gelu_forward_stays_lazy` 断言 forward 链 0 eval + 与 CPU erf 公式对拍 ≤ 1e-4）。✅ M5.3b.9 `gather_last_dim` lazy forward（`mlx_reshape → mlx_take_axis(axis=0, flat ids) → mlx_reshape` 三节点组合；复用 embedding 的 flatten+remap-indices 思路，indices 每 step 上传为 `[prefix]` int32 临时 array（remap 成 `i*vocab+ids[i]`），完成 `src[B,S,V] → picked[B,S]` 的行内 gather；dispatch 同 embedding 模板覆盖 `Dirty::Both`，CE-loss 路径上 `matmul → gather_last_dim → sum` 从 token-level logits 一路驻留设备；backward 仍走 host `scatter_add_rows`，依赖 tape 批量 flush 提供 host upstream；新增 `metal_gather_last_dim_forward_stays_lazy` 断言 forward 链 0 eval + CPU 参考对拍 ≤ 1e-5）。✅ M5.3b.10 `AdamW` on device（`Backend::adamw_step` trait method 默认走 readback→host→upload fallback，Metal override 把整条更新组合进 MLX lazy 图：upload grad 后 `β1·m + (1-β1)·g` / `β2·v + (1-β2)·g²` 两条乘-加链→ `m_hat = new_m · (1/bc1)`、`v_hat = new_v · (1/bc2)`→ `sqrt(v_hat) + eps` 用 `mlx_sqrt` + `mlx_add`→ `ratio = m_hat · reciprocal(denom)`（`mlx_divide` FFI 不存在，用 `mlx_reciprocal` 组合）→ `scaled_update = lr · ratio`→ `decayed_param = (1-lr·wd) · param`→ `new_param = decayed_param - scaled_update`，单个终端 `mlx_eval([new_param, new_m, new_v])`；新增 `MomentStorage::Host(Vec<f32>) / Device(DeviceHandle)` 存放 m/v，`AdamW::new_with_device(lr, betas, eps, wd, backend)` 构造器走设备路径，现有 15 个 `AdamW::new` 调用点零改动；新增 `TensorStore::replace_device_handle(id, handle)` 绕开 `get_mut` 的 `ensure_host` 自触发（否则会把刚算出的 new_param 立刻下拉回 host，正是要消灭的 churn）；Qwen3.5 级模型 ~200 参数 × param-size 每 step CPU↔GPU 往返消除；新增 `metal_adamw_step_stays_device_resident` 断言 5 步 L2 diff ≤ 1e-5 + `Dirty::Device` 驻留 + per-step eval delta ≤ 2，实测观测值为 1/step）。✅ M5.3b.19 `mean` lazy forward（不加新 trait method，`ops::reduce::mean` 直接复用已有 lazy primitives 组合：Dirty::Device 路径 `store.backend().sum_all(&input_handle, &input_shape)`（M5.3b.1 lazy sum） → `store.backend().mul_scalar(&sum_handle, 1.0/numel, &[])`（M5.3b.13 lazy rank-0 multiply），两个 FFI 调用组合进 MLX 图 0 eval；numel=0 guard 避免 div-by-zero；`ops::mean` 拆成 dispatcher + `mean_device_lazy` + `mean_host_eager`，Dirty::Host/Dirty::Both 走旧 host 路径；`ops.rs` 公共入口剥掉 `ensure_host`；`mean_backward` 无需改动（只读 saved numel 和 output_grad 的 .data[0] 标量）；**这是 M5.3b 系列最后一个（也是最高价值的）隐形 chokepoint**：`ops::mean` 在 `crates/train/src/loss.rs:14` 被 CE loss 调用，pre-M5.3b.19 每训练步 flush 整块 `[batch,seq]` log_probs 回 host 只为算一个标量 loss，反噬 M5.3b.1–18 所有上游 lazy 保留；移除后 CE-loss head `log_softmax → gather_last_dim → mean → mul_scalar(-1)` 端到端 lazy；新增 `metal_mean_forward_stays_lazy` 断言 `matmul → mean` 链 forward 0 eval + backward ≤3 + CPU 参考对拍 ≤ 1e-4；autograd metal 25 路 (+1) + train 36+ 组全绿；pending-remote 预计为判定 M5.3b 系列工程价值的最终 benchmark）。✅ M5.3b.18 `sigmoid` lazy forward（`Backend::sigmoid_forward(a) -> Vec<f32>`（默认 `cpu_sigmoid_forward`）+ `Backend::sigmoid(x, shape) -> DeviceHandle` trait method 默认 readback→`sigmoid_forward`→upload fallback；Metal override 一次 `mlx_sigmoid(x_handle)` FFI 调用；`ops::activation::sigmoid` 拆成 Dirty dispatcher + `sigmoid_device_lazy` + `sigmoid_host_eager`，dispatch 条件 `device_handle.is_some() && dirty != Host`（覆盖 Dirty::Device + Dirty::Both）；`ops.rs` 公共入口剥掉 `ensure_host`；`sigmoid_backward` 用 `SavedContext::SigmoidCtx { y: output_id }` 保存输出，安全性依赖 `tape.backward` pre-walk `flush_to_host_batch`；Qwen3.5 attention `gate = sigmoid(gate_full)`（`qwen35.rs:142`）× 28 层 = 28 evals/token 潜在消除；M5.3b.16 `slice(gate)` → M5.3b.18 `sigmoid` → M5.3b.17 `mul(attn, gate)` 现在整条 attention gate 链端到端 lazy；新增 `metal_sigmoid_forward_stays_lazy` 断言 `matmul → sigmoid → sum` 链 forward 0 eval + backward ≤3 + CPU 参考对拍 ≤ 1e-4；autograd metal 24 路 (+1) + train 36+ 组全绿）。✅ M5.3b.17 elementwise `mul` lazy forward（`Backend::mul(a, b, shape) -> DeviceHandle` trait method 默认 readback→`mul_forward`→upload fallback；Metal override 一次 `mlx_multiply(a_handle, b_handle)` FFI 调用（形状由 caller 保证相等 —— 老 `ops::mul` 契约沿用）；`ops::elementwise::mul` 拆成 OR-lazy dispatcher + `mul_device_lazy` + `mul_host_eager`，dispatch 条件 `a_lazy \|\| b_lazy`（**OR 不是 AND**，与 `add_broadcast` 同构：hot path 两侧都 Dirty::Device 从上游 matmul/sigmoid 链出，若 AND 会因"只有一个 lazy"退回 host_eager 反噬 matmul lazy 图；OR 作保险丝永不退化，upload 小端比 readback 大端廉价）；`ops.rs` 公共入口剥掉 `ensure_host(a)+ensure_host(b)`；`mul_backward` 用 `SavedContext::Tensors([a,b])` 保存两侧 + `.clone()` 读输入，安全性依赖 `tape.backward` pre-walk `flush_to_host_batch` 把所有上游 entry output 批量置 `Dirty::Both`；Qwen3.5 每 attention 层一处 gated multiply（`attn * sigmoid(gate)`）+ 每 MLP 层一处 SwiGLU activation（`silu(gate) * up`）共 2 次/层 × 28 层 = 56 evals/token 潜在消除；新增 `metal_mul_forward_stays_lazy` 断言 `matmul(a·wa) * matmul(b·wb) → sum` 链 forward 0 eval + backward ≤ 6 + CPU 参考对拍 ≤ 1e-4；autograd metal 23 路 (+1) + train 30 组全绿；pending-remote：训练箱 Qwen3.5-4B `METAL_EVAL_COUNT` 对拍 pre-M5.3b.17 vs 当前）。✅ M5.3b.16 `slice` lazy forward（`Backend::slice(x, old_shape, starts, ends) -> DeviceHandle` trait method 默认 readback→`cpu_slice`→upload fallback；Metal override 走 `mlx_slice(strides=all 1) → mlx_contiguous`，**关键**：`mlx_slice` 返回非连续 view（stride/offset 层面的元数据变换），`mlx_array_data_float32` readback 忽略视图信息会读到原布局 bytes —— 跟 M5.3b.12 transpose 同族陷阱，修复同样是包 `mlx_contiguous` 强制 eval 时窗口布局物化（MLX 对已连续 array 短路，常见 case 零开销）；`ops::layout::slice` 拆成 dispatcher + `slice_device_lazy` + `slice_host_eager`，dispatch 门同 reshape/transpose/rope；`ops.rs` 公共入口剥掉 `ensure_host`；Qwen3.5 每层 attention 的 q_proj 实际输出 `[batch,seq,heads,head_dim*2]`，切成 `q = q_full[..., :head_dim]` + `gate = q_full[..., head_dim:]`，每层 2 次 slice × 28 层 = 56 evals/token 潜在消除；新增 `metal_slice_forward_stays_lazy` 断言 `matmul → reshape → slice(q)+slice(gate) → sum` 链 0 eval + CPU 参考对拍 ≤ 1e-4；新 pub `cpu_slice` 作为 device-default + host-path 共享数值 reference）。✅ M5.3b.15 `causal_sdpa` 复合 op lazy 收尾（`ops.rs` 公共入口的 `causal_sdpa` 剥掉三个 `ensure_host(q/k/v)?`，body 纯 dispatch `reshape → transpose → matmul → mul_scalar → add_broadcast(causal_mask) → softmax → matmul → reshape`，所有 inner op 在 M5.3b.1–14 已 lazy 化，因此复合 op 一次性端到端保留在 MLX 图内；Qwen3.5 attention 链路 × 28 层每 token 端到端进入 device graph，不再 per-layer readback；新增 `metal_causal_sdpa_forward_stays_lazy` 断言 `causal_sdpa → sum` forward 0 eval + backward ≤ 6；既有 21 路 device-handle 测试 + train 36 路全绿）。✅ M5.3b.14 `add_broadcast` lazy forward（`Backend::add_broadcast(a_handle, a_shape, b_handle, b_shape) -> DeviceHandle` trait method 默认 readback→upload fallback；Metal override 直接复用 `mlx_add`（MLX 的 add 已原生 NumPy right-aligned broadcast，无需显式 `mlx_broadcast_to`）；`ops/broadcast::add_broadcast` 拆成 dispatcher + `add_broadcast_device_lazy` + `add_broadcast_host_eager`，**设计要点**：dispatch 放宽到 `a_lazy \|\| b_lazy`（OR，不是 AND），因为 attention mask 是 host 常量而 scores 是 device 产出，若走 AND 会强制 readback scores 反噬 matmul 的 lazy；host-eager 路径内部加 `ensure_host` 同步（应对 CPU 后端仍用 `alloc_device_tensor` 下游 Linear bias 的混合场景）；`ops.rs` 公共入口剥掉 `ensure_host`；Qwen3.5 attention 每层 causal-mask add + Linear bias add × 28 层潜在消除大量 evals/token；新增 `metal_add_broadcast_forward_stays_lazy` 断言 `matmul → reshape → add_broadcast(mask) → sum` 链 0 eval + CPU 参考对拍 ≤ 1e-5（-inf 位只校验符号+∞性）；module::tests::linear_forward* 两例曾 panic "ensure_host before cloning a device-resident tensor"，根因 CPU 后端 matmul 也走 `alloc_device_tensor`→bias 走 host_eager→clone 触发 assert；修复为 host_eager 开头显式 `ensure_host(a)?+ensure_host(b)?`）。✅ M5.3b.13 `mul_scalar` lazy forward（`Backend::mul_scalar(x, s, shape) -> DeviceHandle` trait method 默认 readback→upload fallback，Metal override 用 `mlx_array_new_float32(s)` 拿 rank-0 scalar + `mlx_multiply(x, scalar)`（MLX 自动 rank-0 broadcast），一次 FFI 调用；`ops/elementwise::mul_scalar` 拆成 dispatcher + `mul_scalar_device_lazy` + `mul_scalar_host_eager`，Dirty::Device/Both → 走 lazy；`ops.rs` 公共入口剥掉 `ensure_host`；Qwen3.5 attention 每层 q 乘 `1/sqrt(d_head)` × 28 层 = 28 evals/token 潜在消除（原先每层 attention 前都要把 q matmul flush 回 host 再 rescale）；新增 `metal_mul_scalar_forward_stays_lazy` 断言 `matmul → mul_scalar → sum` 链 0 eval + CPU 参考对拍 ≤ 1e-5）。✅ M5.3b.12 `reshape` + `transpose_axes_swap` lazy forward（`Backend::reshape` / `transpose_axes_swap` trait method 默认 readback→upload fallback；Metal override `reshape` 走 `mlx_reshape` 元数据节点，`transpose_axes_swap` 走 `mlx_transpose_axes`，**关键**：transpose 返回的是非连续 view，直接 readback 会拿到原始布局的 bytes（实测 parity fail：`metal=-0.4118606 cpu=-0.31485343`），修复为包一层 `mlx_contiguous` 强制 eval 时在新布局物化（MLX 对已连续数组短路，common case 零成本）；`ops::layout::{reshape, transpose}` 剥掉 public entry 的 `store.ensure_host(x)?` 并按 `device_handle.is_some() && dirty != Host` dispatch 到 lazy；Qwen3.5 attention 路径每层 q/k/v projection 后 6 次 reshape + 3 次 transpose × 28 层 = 252 evals/token 潜在消除；新增 `metal_reshape_forward_stays_lazy` / `metal_transpose_forward_stays_lazy` 断言 forward 链 0 eval + CPU 参考对拍 ≤ 1e-4；既有 `metal_rope_forward_stays_lazy` 等 attention-path 测试不变）。✅ M5.3b.11 AdamW 终端 eval 批量化（`backend_metal::adamw_step` 剥掉 per-call `mlx_eval + bump_eval_count`，trait docstring 明确契约：实现 MUST 返回未 eval 的 handle，caller 批一次；`AdamW::step_device` 循环里收集每个 param 的 `(new_param, new_m, new_v)` Arc 克隆进 `pending_eval: Vec<DeviceHandle>`，循环后一次 `backend.eval(&refs)` 覆盖所有 params；CPU 默认 `Backend::eval` 仍 no-op；Qwen3.5 级 ~200 params 每 step 的 AdamW eval 从线性 ~200 降到常数 1；新增 `metal_adamw_step_batches_eval_across_params` 断言 N=8 的 eval 数 ≤ N=1 的 eval 数 + 1（N=1 实测 1，N=8 实测 1）；既有 `metal_adamw_step_stays_device_resident` 单 param 场景 per-step delta ≤ 2 不变）。剩余：eval_count 目标收敛到严格 1 per step（当前 ≤3：1 tape 批量 flush + 1 matmul_backward FFI eval + 1 log_softmax backward host-mul readback slack，后者需 softmax backward 上设备才能归零）。 | 架构就位后每个 op 数值对拍 CPU f64 参考 ≤ 1e-3 |
