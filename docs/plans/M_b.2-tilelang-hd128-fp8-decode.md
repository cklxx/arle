...
- Phase 1 nsys trace `fdb531b`:`decode_attention_varlen_quantized_partial<128, 1, 0>` = 41.6% wall, 4.55 ms × 4104 calls.
- split-step pure-prefill 15.4 µs/token vs split 154 µs/token (`4a3612b`) → 10× tax.
- H2 math:avg attended KV 1024 (chunk-1) vs 3071 (chunk-2) → 3× attention work explains ~3× of 10×, residual 3.34× was the Mixed-disable fallback (per `28056b9`).
- vLLM s14 control 614 tok/s vs ARLE 462 tok/s @ s48 = 75.1% of vLLM (gap = 33%).
