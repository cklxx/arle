...
|---|---|---|---|---|
| P0 page_size=16 | **M0.3** | Unchanged intent. Per-format dispatch still applies (BF16 → 16, quantized → stay at 1). The kernel-crate path move is now absorbed into the landed local diff. | **accepted 2026-04-15 on L4** (`wins/2026-04-15-tiered-kv-m0.3-m3a-remote.md`) | §1 below |
...
- **2026-04-23**: Local CUDA follow-up added a direct T1 staged-readmission fast path plus deferred slower-tier prefetch on the scheduler waiting path. Focused traces measured `T1 staged-prefix ready = 16.3ms`, `T2 staged-prefix ready = 37.6ms`, and logged a real deferred prefetch (`queued -> materialized -> host-backed replay`) before admission. The canonical `Qwen3-4B` / L4 `c16` lane remained tier-cold and stayed within noise at `118.32 tok/s` vs the prior `119.14 tok/s` baseline. See `docs/experience/wins/2026-04-23-profile-kv-tier-qwen3-t1-t2-prefetch-latency.md` and `docs/experience/wins/2026-04-23-bench-guidellm-qwen3-4b-l4-c16-tier-prefetch-42ce889.md`.
