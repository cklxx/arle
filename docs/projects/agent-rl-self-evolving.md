...
> 当前 train-side 训练模型线已经变成通用 Qwen-family 控制面，且以 Qwen3.5 dense/full-attn 为默认与优化主线；`train_multi_turn` 已经支持 stepwise GRPO 和 sequence-level GSPO 两种 objective，并支持 exact checkpoint/resume，但 hybrid linear-attn 仍是后续目标，不是已经完成的现状。
