...
这一节记录把 8 并发吞吐从 128 tok/s 优化到 811 tok/s 的完整过程，以及和 SGLang 对比的 nsys 分析。
...
2. **SGLang 用了 FusedAddRMSNorm**：把 residual add 和 RMSNorm 合成一个 kernel，省掉一次全局内存读写。我们在 batched path 是分开的（`add_batch_into` + `rms_norm_batch_into`），这是下一个优化目标。
...
1. **CUDA Graphs**（~50 行）— 我们 ✅ 已实现
2. **FlashInfer Plan 缓存**（~10 行）— CPU 端算 cumsum 给 FlashInfer，避免 plan 内部的 H2D sync
3. **Overlap Scheduling**（~100 行）— 用独立 CUDA stream 流水线化 CPU + GPU 工作
4. **GPU-side metadata**（~40 行）— 用 Triton kernel 在 GPU 端计算 KV indices，不用 H2D
5. **Pinned Memory**（~20 行）— cudaHostRegister 给 H2D buffer，120GB/s vs 40GB/s

### 剩余差距的优化方向

| 优化 | 预期收益 | 复杂度 |
|------|----------|--------|
| FusedAddRMSNorm（batched） | ~0.3ms/step | 需要新 CUDA kernel |
| 更快的 argmax kernel | ~0.1ms/step | 优化 warp reduction |
| cudaEventSync 替代 cuStreamSync | ~0.2ms/step | 改 cudarc 用法 |
| GPU-side KV indices 更新 | ~0.1ms/step | Triton/CUDA kernel |
| Pinned memory for H2D | ~0.05ms/step | cuMemHostRegister |
| Overlap scheduling（双 stream） | ~0.5ms/step | 架构级改动 |
...
**FusedAddRMSNorm + Overlap scheduling**：这两个优化是缩小剩余 10% 差距的关键。前者是 kernel 级优化，后者是调度级优化。
