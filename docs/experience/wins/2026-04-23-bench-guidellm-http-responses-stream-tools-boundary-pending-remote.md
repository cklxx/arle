# HTTP responses streaming tools boundary pending remote verification

## Goal

Stop advertising a false streaming compatibility path by rejecting
`POST /v1/responses` requests that combine `stream=true` with tool
definitions until the server can emit structured function-call streaming
events instead of text-only deltas plus a terminal response snapshot.
...
- Label: `http-responses-stream-tools-boundary`
- Planned command: `scripts/bench_guidellm.sh http-responses-stream-tools-boundary`
...
- A streaming Responses endpoint should not accept tool-calling requests
  unless it can emit structured function-call progress events rather than
  only text deltas and a terminal completed snapshot.
