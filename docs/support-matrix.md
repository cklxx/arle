...
| Hybrid linear-attn Qwen3.5 scratch pretrain / RL acceptance | Partial | Scratch pretrain remains dense/full-attn only, and `train_grpo` / `train_multi_turn` still need end-to-end acceptance on hybrid checkpoints before this surface is treated as fully shipped. |
