...
use train::model::{Transformer, TransformerConfig};
...
fn perturb_params(store: &mut TensorStore, model: &Transformer, scale: f32) {
...
fn snapshot(store: &TensorStore, model: &Transformer) -> Vec<Vec<f32>> {
    model
        .parameters()
        .iter()
...
fn save_then_load_restores_exact_parameter_values() {
    ...
    let model = Transformer::new(config, &mut store).expect("model");
...
fn bad_magic_is_rejected() {
    ...
    let mut store = TensorStore::default();
    let model = Transformer::new(tiny_config(), &mut store).expect("model");
