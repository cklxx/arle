// FlashInfer tensor-core decode wrapper for Rust FFI
//
// Uses BatchPrefillWithPagedKVCacheDispatched (with qo_len=1 per request)
// instead of BatchDecodeWithPagedKVCacheDispatched. The prefill kernel tiles
// across KV chunks, keeping ITL flat as context grows.
//
// This matches sglang's "use_tensor_cores=True" path for GQA group_size >= 4.
...
// ---------------------------------------------------------------------------
// flashinfer_tc_decode_plan  –  CPU-side scheduling (PrefillPlan)
// ---------------------------------------------------------------------------
extern "C" int flashinfer_tc_decode_plan(
    void*         float_workspace,
    size_t        float_workspace_bytes,
    void*         int_workspace,
    void*         page_locked_workspace,
    size_t        int_workspace_bytes,
    int32_t*      qo_indptr_h,           // [batch_size+1] host, e.g. [0,1,2,...,B]
    int32_t*      kv_indptr_h,           // [batch_size+1] host (KV token counts)
    int           batch_size,
    int           num_qo_heads,
    int           num_kv_heads,
    int           page_size,
    int           head_dim,
    void*         plan_info_out,          // opaque buffer >= sizeof(PrefillPlanInfo)
    cudaStream_t  stream)
{
    ...
    cudaError_t err = flashinfer::PrefillPlan<IdType>(
        ...
        total_num_rows, (uint32_t)batch_size,
...
// ---------------------------------------------------------------------------
// flashinfer_tc_decode_run  –  launch GPU kernel
// ---------------------------------------------------------------------------
extern "C" int flashinfer_tc_decode_run(
    void*          float_workspace,
    void*          int_workspace,
    void*          plan_info_ptr,
    __nv_bfloat16* q,
    int32_t*       q_indptr,              // GPU [batch_size+1]
    ...
    cudaStream_t   stream)
{
    ...
    // Build paged_kv_t (HND layout, matching our KV cache)
    ...
    params.qo_tile_indices = flashinfer::GetPtrFromBaseOffset<IdType>(
        int_workspace, plan_info.qo_tile_indices_offset);
    ...
    if (plan_info.split_kv) {
        params.merge_indptr = flashinfer::GetPtrFromBaseOffset<IdType>(
            int_workspace, plan_info.merge_indptr_offset);
    ...
    // Dispatch with CTA_TILE_Q from plan
    cudaError_t err;
    uint32_t cta_tile_q = plan_info.cta_tile_q;

    // CTA_TILE_Q is a compile-time constant — dispatch on common values
#define DISPATCH_TC_DECODE(CTQ)                                             \
    err = flashinfer::BatchPrefillWithPagedKVCacheDispatched<               \
        CTQ, HEAD_DIM, HEAD_DIM, POS_MODE, USE_FP16_QK_REDUCTION,         \
        MASK_MODE, Variant, Params>(                                        \
        params, tmp_v, tmp_s, /*enable_pdl=*/false, stream);

    switch (cta_tile_q) {
        case 16: DISPATCH_TC_DECODE(16); break;
        case 32: DISPATCH_TC_DECODE(32); break;
        case 64: DISPATCH_TC_DECODE(64); break;
        case 128: DISPATCH_TC_DECODE(128); break;
        default:
            // Fallback: use CTA_TILE_Q=16 (minimum)
            DISPATCH_TC_DECODE(16);
            break;
    }
#undef DISPATCH_TC_DECODE
