...
extern "C" int flashinfer_batch_prefill_paged_hd128_plan(
  ...
  try {
    ...
    std::fprintf(stderr, "flashinfer_batch_prefill_paged_hd128_plan threw: %s\n", e.what());
    return -1;
  } catch (...) {
    std::fprintf(stderr, "flashinfer_batch_prefill_paged_hd128_plan threw unknown\n");
    return -1;
  }
}

extern "C" int flashinfer_batch_prefill_paged_hd128_run(
    void* float_workspace,
    void* int_workspace,
    void* plan_info_ptr,
    __nv_bfloat16* q,
  ...
  try {
    ...
    // FlashInfer reads `total_num_rows` from device memory when the plan
    // was created with `enable_cuda_graph=true`; without this pointer wire
    // the dispatched kernel does OOB reads on the Q tensor.
...
#define DISPATCH_BATCH_PREFILL_PAGED_HD128(CTQ)                                      \
  err = flashinfer::BatchPrefillWithPagedKVCacheDispatched<CTQ,                      \
    ...
    switch (cta_tile_q) {
      case 16:
        DISPATCH_BATCH_PREFILL_PAGED_HD128(16);
        break;
      case 32:
        DISPATCH_BATCH_PREFILL_PAGED_HD128(32);
        break;
      case 64:
        DISPATCH_BATCH_PREFILL_PAGED_HD128(64);
        break;
      case 128:
        DISPATCH_BATCH_PREFILL_PAGED_HD128(128);
        break;
      default:
        DISPATCH_BATCH_PREFILL_PAGED_HD128(16);
        break;
...
#undef DISPATCH_BATCH_PREFILL_PAGED_HD128

    return static_cast<int>(err);
  } catch (const std::exception& e) {
    std::fprintf(stderr, "flashinfer_batch_prefill_paged_hd128_run threw: %s\n", e.what());
    return -1;
  } catch (...) {
    std::fprintf(stderr, "flashinfer_batch_prefill_paged_hd128_run threw unknown\n");
    return -1;
  }
}
